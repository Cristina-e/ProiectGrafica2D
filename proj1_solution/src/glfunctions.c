...
{
	char str[64];
	sprintf(str, "Vieti: %2d", g_pesteJucator->campuriPeste.pesteJucator.nr_vieti);
...
{
	char str[64];
	sprintf(str, "Scor: %2d", g_pesteJucator->campuriPeste.pesteJucator.scor_curent);
...
{
	char str[64];
	sprintf(str, "Timp scurs: %02d:%02d", timer.minutes, timer.seconds);
...
void showExitScreen(void)
{
	...
	for (int i = 1; i <= N_CERCURI_NISIP; i++)
	{
		glColor3ub(cercuri_nisip[i].culoare.red, cercuri_nisip[i].culoare.blue, cercuri_nisip[i].culoare.green);
		drawCercuriNisip(cercuri_nisip[i].xPos, cercuri_nisip[i].yPos, cercuri_nisip[i].radius);
	}
